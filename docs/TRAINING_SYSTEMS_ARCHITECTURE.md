...
| 4 | **FrameworkBackend** | compute substrate (PyTorch / JAX / TF-Keras / MLX) | **contract shipped (P0c, [#483](https://github.com/MalloyTheDev/CorpusStudio/issues/483))** - `FrameworkBackend` split from `BackendManifest` **append-only** (its digest is untouched) + registry + `SupportLevel`. Defaults evidence-selected. |
| 5 | **OrchestratorAdapter** | the training-loop driver (HF/TRL, Unsloth, torchtune, Axolotl, Megatron, ...) | **contract shipped (P0c, #483)** - `OrchestratorAdapter` + `BackendSecurityPosture` (`trust_remote_code`/network/download), refusable by `AssuranceTier` (`refuse_backend_security`). |
...
**Implemented (P0a, [#481](https://github.com/MalloyTheDev/CorpusStudio/issues/481)):**
