"""The DPO execution seal (S2b-1): ``ResolvedPreferenceExecutionConfiguration`` is the sibling of the
byte-locked dense SFT seal for the ``preference_dpo`` variant. It must be expressible + hash-sealable
(so the architecture is reviewable before any worker byte changes), fail closed on the DPO-specific
invariants, and NEVER perturb the dense SFT seal. It is not yet consumed by a worker - the DPOTrainer
branch + a workload-verified run + the milestone wheel are a separate, gated slice."""
