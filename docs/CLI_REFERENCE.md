...
| `env-plan <recipe-id> [--accelerator cu128\|cpu\|…] [--python 3.12] [--json]` | **Preview** provisioning a recipe on this host — the exact **argv install steps** (never a shell string), the CUDA-aware wheel index (a Blackwell host → the `cu128` index), and the disk/network cost — **without installing anything** (env *creation* is a later slice). Reports `resolvable=false` with reasons when the host can't satisfy it (unmet Python floor / unsupported OS / CUDA required but absent). |
