...
def test_all_root_contracts_registered():
    assert len(P.ROOT_CONTRACTS) == 44
    assert "ResolvedPreferenceExecutionConfiguration" in P.ROOT_CONTRACTS
    assert "ResolvedRewardExecutionConfiguration" in P.ROOT_CONTRACTS
    assert "ResolvedFullFinetuneExecutionConfiguration" in P.ROOT_CONTRACTS
    ...
    assert "SandboxPolicy" in P.ROOT_CONTRACTS
    for expected in (
        "ModelDescriptor",
...
def test_export_json_schemas_writes_language_neutral_files(tmp_path):
    written = P.export_json_schemas(tmp_path)  # ROOT_CONTRACTS + the index file
    # contract schemas + index.json
    assert len(written) == 45
    index = json.loads((tmp_path / "index.json").read_text(encoding="utf-8"))
    assert index["contract_version"] == "1.0.0"
    assert len(index["contracts"]) == 44
