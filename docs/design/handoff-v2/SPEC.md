...
- `README.md` — this document (design tokens, shell, IA, every screen, interactions, state).
- `Corpus Studio.dc.html` — the full interactive design reference (all screens, both themes).
- `assets/icons/<weight>/*.svg` — every icon used, as raw SVG (Phosphor 2.1.1).
- `assets/ICONS.md` — icon inventory + React usage + font notes.
- `assets/icons.json` — machine-readable icon manifest (name → React import → weights → role).
