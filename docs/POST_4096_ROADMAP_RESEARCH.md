...
| **2** | **[SHIPPED - #618 ledger, #624 wired into the worker preflight]** Token-coverage ledger (`TokenCoverageLedger`/`compute_token_coverage`/`token_coverage_refusal`): tokens + supervised-label coverage, `boundary_severances`, refuse gate on dropped supervised tokens. Pure control-plane, torch-free. | Closes the "counts examples not tokens" gap; prerequisite for honest long-context. Done. | S-M |
| **3** | **[SHIPPED - #625 `splitters/structure_aware_chunker.py`; not yet wired into a CLI command]** Structure-aware chunker: message/turn-boundary split, single-span-over-seqlen → refuse surfaced. Remaining: a CLI/worker entry point + a token_len that reflects the full chat-template render. | The honest long-context default; depends on #2's ledger. | M |
