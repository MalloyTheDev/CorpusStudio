...
- Validation currently enforces JSON object rows, required non-empty fields, declared field types, chat message structure, list element types (`item_type`, e.g. `tags` must be a list of strings), and fixed value sets (`enum`) for scalar fields.
