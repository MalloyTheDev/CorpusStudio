...
- **Full-sequence 7B training success as a SEALED result** — the probe is a minimal kernel check,
  ...
  lineage) was proven on real worker bytes - built reproducibly into a worker wheel
  (`corpus_studio_engine-1.3.0`, sha `2f792a37`, source `9c32b4d`, ancestor `origin/main`), installed
  into a `HARDWARE_VERIFIED` env, and a **7B (Qwen2.5-7B nf4) checkpoint->resume ran on that wheel's own
  bytes** - a FRESH resume process from a step-2 checkpoint reproduced the uninterrupted loss trajectory
  bit-for-bit (step3 2.4404, step4 2.2823; `resumed_from_step=2`, `step_losses=[3,4]`). This is a
  dev-milestone EVIDENCE artifact for exactly ONE training tuple (dense-QLoRA-SFT + resume) - NOT "the
  product wheel/env" and NOT a sealed IEEE cell; the `SupportLevel` ladder is tuple-scoped, so every
  other paradigm stays unproven. Product build-out iterates in DEV mode; a worker wheel is pinned + an
  env sealed only at a promotion or a product release, never per training-method change.
