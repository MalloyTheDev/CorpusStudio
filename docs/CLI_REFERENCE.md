...
| `training-objectives [OBJECTIVE_ID] [--json]` | List the 29 versioned, hash-sealed objective definitions or show one complete `TrainingObjective`. Definitions cover dataset fields, labels, masks, separately keyed losses, model/update/backend requirements, artifacts, resume/eval/hardware implications, limitations, and verification; registry presence is not backend support. See [TRAINING_SYSTEMS_ARCHITECTURE.md](TRAINING_SYSTEMS_ARCHITECTURE.md). |
