...
| `observe.py` | Reads the repo via `cs_assure verify` + `doclint` and maps the sealed result to one **mechanical** `Observation` (gate step → failure class; obligations → human-gated / worker / drift). The verify result is a **validated record**, not raw JSON: a refusal exit (≥2), a wrong `record_type`, an unsupported schema (< v2), a missing change-set fingerprint, or `workspace_stable != True` (the tree mutated during the gate) each fail closed; an unavailable `doclint` is surfaced explicitly, never silently read as clean. |
