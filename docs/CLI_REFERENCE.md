...
| Command | What it does |
|---|---|
| `eval-run <file.jsonl> <schema> --model … [--backend … --base-url … --judge-model … --judge-backend … --judge-base-url … --output-schema … --seed … --temperature … --max-output-tokens … --limit … --progress --reasoning]` | Run one Evaluation Studio pass; `--progress` streams `[k/N] evaluated` to stderr. **`--reasoning`** scores a reasoning model's **answer only** — the `<think>…</think>` block is stripped before scoring (the reasoning isn't the reference) while the full output is kept in the record; a "reasoning" model that emitted no reasoning is flagged. **`--output-schema <schema-id>`** switches to the deterministic `schema_conformance` scorer (JSON structural match against a builtin `DatasetSchema`; mutually exclusive with `--judge-model`). **`--seed/--temperature/--max-output-tokens`** pin decoding for reproducibility and are recorded in the run settings. |
...
| `platform-plan --base-model … --model-revision IMMUTABLE_COMMIT --dataset … [--task-type sft\|preference --objective ID --tokenizer-revision COMMIT --sequence-len N --micro-batch-size N --gradient-accumulation-steps N --max-steps N\|--epochs N --dpo-beta N --dpo-label-smoothing N --max-prompt-length N --allow-truncation --chat-template-sha256 HASH --backend ID --environment ID --manager-root DIR --allow-cpu-toy --json --out DIR]` | Resolve an immutable, **hash-sealed RunPlan** plus separately sealed `ResolvedExecutionConfiguration` (or, for `--task-type preference --objective dpo_qlora`, a sealed `ResolvedPreferenceExecutionConfiguration` - admitted at planning, refused at execution until the DPO worker is workload-verified; `--dpo-beta`/`--dpo-label-smoothing`/`--max-prompt-length` are fail-closed DPO knobs). Hub inputs require immutable commits; local model/tokenizer directories and the dataset file are content-hashed. The configuration seals per-state precision, exact attention API/kernel/all SDP toggles, an explicit device map, semantic LoRA/trainer/checkpoint/data defaults, and the exact trainer interface, including `logging_strategy="steps"`, `logging_steps=1`, and `logging_nan_inf_filter=false`. `--micro-batch-size` (default 1) and `--gradient-accumulation-steps` (default 8) seal the exact batching tuple consumed verbatim by the worker; both must be positive, and omitting them preserves the product-wide defaults. New first-party plans explicitly disable intermediate checkpoints (`save_strategy="no"`, no cadence or retention); non-null checkpoint requests are refused until exact resume lineage exists. Admission requires one matching passing execution-combination probe plus trainer-surface proof; explicit overrides and independently passing axes cannot bypass it. A managed package claiming verified integrity must have complete positive RECORD/verified/installed-file counts. Unsloth currently lacks this Phase 9B declaration/proof and is refused on every host. `--environment` live-checks the managed lock and builds proof inside its interpreter. Prints the predicted **fit** (never `NATIVE_SAFE` from an estimate); `--json` emits `{run_plan, fit_classification}`. |
...
| `checkpoint-verify <checkpoint> [--json]` | Verify a sealed checkpoint (manifest, bound identities, per-file hashes, completeness marker) without resuming - a read-only integrity check. See [CHECKPOINT_RESUME.md](CHECKPOINT_RESUME.md). |
| `telemetry-summarize <run-dir\|RunEvents.jsonl> [--json --out FILE]` | Aggregate a run's raw `RunEvents` into a `RunTelemetrySummary` (measurement windows, trapezoidal energy, completeness) - torch-free; an unavailable metric is null with a typed reason, never a fabricated zero. See [MEASUREMENT_HARNESS.md](MEASUREMENT_HARNESS.md). |
| `build-worker-wheel [--out DIR --source-commit HEX --required-git-ancestor HEX]` | Build the `[train]` worker wheel with an embedded canonical `BUILD_PROVENANCE.json` (source_commit + reviewed floor). Reproducible: two builds from the same source produce a byte-identical wheel. |
...
| `training-objectives [OBJECTIVE_ID] [--json]` | List the 30 versioned, hash-sealed objective definitions or show one complete `TrainingObjective`. Definitions cover dataset fields, labels, masks, separately keyed losses, model/update/backend requirements, artifacts, resume/eval/hardware implications, limitations, and verification; registry presence is not backend support. See [TRAINING_SYSTEMS_ARCHITECTURE.md](TRAINING_SYSTEMS_ARCHITECTURE.md). |
