...
| `locking.py` | A portable stdlib **advisory file lock** (`O_CREAT\|O_EXCL` lockfile, bounded `timeout`, stale-lock breaking) guarding the cross-process read-modify-write of the shared learning ledger, so two concurrent campaigns cannot lose each other's append. |
