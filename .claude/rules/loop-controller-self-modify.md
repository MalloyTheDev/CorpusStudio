...
A change under `scripts/loop/`, to `scripts/cs_loop.py`, to a loop test, or to `docs/AUTONOMOUS_LOOP.md`
edits the **autonomous loop controller** - the thing that decides retry / stop / escalate / hold / merge
/ finalize. The loop's own run is therefore **not** independent verification of a change to it, exactly as
