...
`index.json` lists all **44 root contracts** and their shared `contract_version`.
...
| `ModelCodeVettingReport` | The auditable admission token for the mode-3 **custom-block** path (your own model code): a content-addressed record of STATICALLY screening a local bundle (`bundle_sha256` pins the exact bytes; `verdict` is `admitted` iff no error findings). A static screen is a pre-screen, **not** a safety proof - it never executes the bundle, never uses `trust_remote_code`, and containment is the (gated) worker sandbox. Produced by `vet-model-code`. |
| `SandboxPolicy` | The OS-level containment for executing vetted-but-untrusted custom-block code (mode-3, slice 3): no network, a read-only root with only run-scoped writable binds, resource limits, and no-new-privileges - three invariants **type-locked** (`Literal[True]`). `gpu_devices` is an honest, documented hole (a GPU workload is blast-radius-limited, not fully isolated). Realized by the `bubblewrap` launcher (an `unshare`+rlimits fallback is weaker); a host with no backend refuses to run custom code. |
