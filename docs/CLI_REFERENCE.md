...
| Command | What it does |
|---|---|
| `env-recipes [--layer control_plane\|capability\|backend_worker] [--json]` | List the built-in **environment recipes** (declarations of what to install): the control-plane core, capability profiles, and isolated backend workers. `backend-corpus-studio-readiness-v2` is the exact-pinned, wheel-bound QLoRA readiness recipe; it remains `declared` until separately created and probed. |
