...
`examples/wbg/README.md`). The **product** lineage has since reproduced this end to end
(2026-07-30) on a `HARDWARE_VERIFIED` v9-**product** env - `NATIVE_TIGHT` measured fit and a passing
held-out completeness eval; canonical run and eval detail live in `examples/wbg/README.md` and
`docs/HOST_STATE.md`. This is exploratory/product evidence: real offload paths remain unverified and
...
- **Provenance gate** (`provenance-gate`): a per-row *licensing* check — reads each row's
  ...
  (e.g. Anthropic/OpenAI/Google Gemini or PaLM), the counterpart to `provider-policy`
  (generation-time). Buckets
