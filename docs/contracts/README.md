...
| `ResolvedPreferenceExecutionConfiguration` | The sibling hash-sealed execution configuration for offline **preference optimization (DPO)** - reuses every shared execution sub-spec and adds a `PreferenceDataPolicy` plus the DPO loss/reference seal; kept separate so the byte-locked SFT seal is never perturbed. |
