...
public sealed class MainWindowViewModel : INotifyPropertyChanged
{
    ...
    private string _provenanceGateSummary =
        "Check whether any rows were generated by a provider you can't train on (reads each row's meta.teacher).";
    private string _provenanceGateStatus = string.Empty;
    private string _provenanceGateColor = GateReport.StatusColor(null);
    private bool _hasProvenanceGateReport;
    private bool _provenanceGateStrict;
    ...
    public System.Windows.Input.ICommand RunProvenanceGateCommand { get; }
        ...
        IProcessRunner trainingRunner)
    {
        ...
        RunProvenanceGateCommand = new AsyncRelayCommand(RunProvenanceGateAsync);
    ...
    /// <summary>Run the per-row provenance gate over the project's dataset and apply the report:
    /// quarantines rows whose declared teacher (meta.teacher) is a provider you can't train on.
    /// Guards an active project + busy state, mirroring the other gate runs.</summary>
    public async System.Threading.Tasks.Task RunProvenanceGateAsync()
    {
        if (!HasActiveProject || string.IsNullOrWhiteSpace(ActiveProjectPath))
        {
            SetProvenanceGateError("Create or select a dataset project before checking provenance.");
            return;
        }

        try
        {
            SetBusy("Checking dataset provenance...");
            SetProvenanceGateInProgress();
            var report = await _engine.RunProvenanceGateAsync(ActiveProjectPath, ProvenanceGateStrict);
            ApplyProvenanceGateReport(report);
        }
        catch (System.Exception ex)
        {
            SetProvenanceGateError(ex.Message);
        }
        finally
        {
            ClearBusy();
    ...
    // Surfaces the engine `provenance-gate`: reads each row's declared teacher (meta.teacher)
    // and quarantines rows generated by a provider you can't train on. The verdict is a
    // licensing/provenance judgment over DECLARED teachers, never a proof of origin.

    public string ProvenanceGateSummary
    {
        get => _provenanceGateSummary;
        private set => SetField(ref _provenanceGateSummary, value);
    }

    /// <summary>The overall verdict word (BLOCK/WARN/PASS/UNKNOWN) for the badge; empty when not yet run.</summary>
    public string ProvenanceGateStatus
    {
        get => _provenanceGateStatus;
        private set => SetField(ref _provenanceGateStatus, value);
    }

    /// <summary>Badge foreground hex (reuses the gate status→color map; unknown is never green).</summary>
    public string ProvenanceGateColor
    {
        get => _provenanceGateColor;
        private set => SetField(ref _provenanceGateColor, value);
    }

    public bool HasProvenanceGateReport
    {
        get => _hasProvenanceGateReport;
        private set => SetField(ref _hasProvenanceGateReport, value);
    }

    /// <summary>Two-way bound: when set, unknown-provenance rows BLOCK too (passes --strict).</summary>
    public bool ProvenanceGateStrict
    {
        get => _provenanceGateStrict;
        set => SetField(ref _provenanceGateStrict, value);
    }

    public void SetProvenanceGateInProgress() => ProvenanceGateSummary = "Checking dataset provenance...";

    /// <summary>Collapse to a neutral error state (never a green pass).</summary>
    public void SetProvenanceGateError(string message)
    {
        ProvenanceGateStatus = string.Empty;
        ProvenanceGateColor = GateReport.StatusColor(null);
        HasProvenanceGateReport = false;
        ProvenanceGateSummary = $"Provenance check could not run.{Environment.NewLine}{message}";
    }

    /// <summary>Render a provenance-gate report into the badge + per-teacher summary. Pure/testable;
    /// unknown/empty status stays neutral gray so an absent verdict can never read as a pass.</summary>
    public void ApplyProvenanceGateReport(ProvenanceGateReport report)
    {
        var status = report.OverallStatus ?? string.Empty;
        ...
        ProvenanceGateStatus = string.IsNullOrWhiteSpace(status) ? "UNKNOWN" : status.ToUpperInvariant();
        ProvenanceGateColor = GateReport.StatusColor(status);

        var lines = new List<string>
        {
            $"{icon} Provenance: {ProvenanceGateStatus} — {report.TotalRows} row(s): "
            + $"{report.TrainableRows} trainable, {report.QuarantinedRows} quarantined, {report.UnknownRows} unknown",
            report.Summary,
            string.Empty,
        };
        foreach (var bucket in report.Buckets)
        {
            var mark = (bucket.Status ?? string.Empty).ToLowerInvariant() switch
            {
            ...
            lines.Add(
                $"{mark} {bucket.RowCount,6}  {bucket.Teacher}"
                + (string.IsNullOrWhiteSpace(bucket.Note) ? string.Empty : $" — {bucket.Note}"));
        }
        lines.Add(string.Empty);
        lines.Add(
            "Honesty: the verdict is a licensing/provenance judgment over each row's DECLARED teacher "
            + "(meta.teacher) — not a proof of origin. Unknown provenance is quarantine-until-verified, not safe.");

        ProvenanceGateSummary = string.Join(Environment.NewLine, lines);
        HasProvenanceGateReport = true;
    }
