...
- **39 root versioned contracts** (`platform/contracts.py`) → deterministic language-neutral JSON
...
| `TrainingObjective` as a distinct registry | ✅ **contract + registry foundation shipped** | 30 sealed objective definitions + conservative dataset/model/backend evidence checker; RunPlan integration and trainer implementation remain future work — see [`TRAINING_SYSTEMS_ARCHITECTURE.md`](TRAINING_SYSTEMS_ARCHITECTURE.md) |
...
| Static MoE model inspection | ✅ **allowlisted config foundation shipped** | hash-pinned Mixtral/Qwen2-MoE/DeepSeek V2/V3 structure and expert-instance counts; runtime, backend, parameter-residency, and fit proof remain future work — see [`MOE_ARCHITECTURE.md`](MOE_ARCHITECTURE.md) |
...
| **4** | ✅ **`TrainingObjective` registry** (objective distinct from backend) | 30 sealed definitions; explicit labels/masks/loss components, router/expert update policy, artifacts/resume/eval/hardware implications, and independent compatibility evidence ([`TRAINING_SYSTEMS_ARCHITECTURE.md`](TRAINING_SYSTEMS_ARCHITECTURE.md)) |
...
| **8** | ✅ **Static MoE model inspection** (parse allowlisted existing-MoE configs; report structural expert-instance counts) | static metadata only; runtime capability remains unverified ([`MOE_ARCHITECTURE.md`](MOE_ARCHITECTURE.md)) |
