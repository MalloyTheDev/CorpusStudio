...
| `store.py` | Durable `LoopState`: deterministic atomic JSON persistence, fail-closed load, crash-resumable. For a **concurrent** (multi-process) writer, `save_cas` adds optimistic concurrency — under a file lock it verifies the on-disk `state_revision` matches what the writer loaded (else `ConcurrentStateWrite`), writing `revision+1` with a `state_digest` / `previous_state_digest` / `writer_id`; the single-writer `save` is unchanged. |
