...
# Parent persistence of a child's admitted ArtifactManifests (and its failure path) is covered in
# test_subprocess_variant_admission.py on a genuine variant terminal: an echo plan can claim no
# artifact, so it can no longer carry one through a succeeded terminal.
